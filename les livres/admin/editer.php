...
<body>
    ...
    <a class="navbar-brand" href="../">Les livres</a>
        ...
        <li class="nav-item">
            <a class="nav-link" aria-current="page" href="../admin/afficher.php">livre</a>
            </li>
            <li class="nav-item">
...
<?php foreach ($lelivre as $livre): ?>
    ...
    <div class="mb-3">
        ...
        <input type="number" class="form-control" name="prix" value="<?= $livre->prix ?>" required>
    </div>
    <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">L'image du produit</label>
        <input type="name" class="form-control" name="image" value="<?= $livre->image ?>" required>

    </div>
    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Description</label>
        <textarea class="form-control" name="description" required><?= $livre->description ?></textarea>
    </div>
...
<?php
    ...
    if(isset($_POST['valider']))
    {
        ...
        if( !empty($_POST['nom']) AND !empty($_POST['prix']) AND  !empty($_POST['image']) AND !empty($_POST['description']))
            ...
            try 
            {
                modifier( $nom, $prix, $image,  $description, $id);
                header('Location: afficher.php');
            } 
            catch (Exception $e) 
            {
