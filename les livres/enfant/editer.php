...
if(isset($_GET['id'])){
    ...
    {
        $id = $_GET['id'];
        $lelivre = afficherUnProduitenfant($id);
    }
}
...
<body>
    ...
    <a class="navbar-brand" href="enfant.php">Les livres d'enfants</a>
        ...
        <li class="nav-item">
            ...
            <li class="nav-item">
            <a class="nav-link" aria-current="page" href="../enfant/">Nouveau</a>
            </li>
            <li class="nav-item">
...
<?php foreach ($lelivre as $enfant): ?>
        
    <form method="post">
    
    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Nom du produit</label>
        <input type="text" class="form-control" name="nom" value="<?= $enfant->nom ?>"  required>
    </div>

    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Prix</label>
        <input type="number" class="form-control" name="prix" value="<?= $enfant->prix ?>" required>
    </div>
    <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">L'image du produit</label>
        <input type="name" class="form-control" name="image" value="<?= $enfant->image ?>" required>

    </div>
    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Description</label>
        <textarea class="form-control" name="description" required><?= $enfant->description ?></textarea>
    </div>
...
<?php
    ...
    if(isset($_POST['valider']))
    {
        ...
        if( !empty($_POST['nom']) AND !empty($_POST['prix']) AND  !empty($_POST['image']) AND !empty($_POST['description']))
            ...
            try 
            {
                modifierenfant( $nom, $prix, $image,  $description, $id);
                header('Location: afficher.php');
            } 
            catch (Exception $e) 
            {
