...
{
    header("Location: loginroman.php");
}
...
if(isset($_GET['id'])){
    ...
    {
        $id = $_GET['id'];
        $lelivre = afficherUnProduitroman($id);
    }
}
...
<body>
    ...
    <a class="navbar-brand" href="roman.php">Les livres</a>
...
<?php foreach ($lelivre as $roman): ?>
        
    <form method="post">
    
    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Nom du produit</label>
        <input type="text" class="form-control" name="nom" value="<?= $roman->nom ?>"  required>
    </div>

    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Prix</label>
        <input type="number" class="form-control" name="prix" value="<?= $roman->prix ?>" required>
    </div>
    <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">L'image du produit</label>
        <input type="name" class="form-control" name="image" value="<?= $roman->image ?>" required>

    </div>
    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Description</label>
        <textarea class="form-control" name="description" required><?= $roman->description ?></textarea>
    </div>
...
<?php
    ...
    if(isset($_POST['valider']))
    {
        ...
        if( !empty($_POST['nom']) AND !empty($_POST['prix']) AND  !empty($_POST['image']) AND !empty($_POST['description']))
            ...
            try 
            {
                modifierroman( $nom, $prix, $image,  $description, $id);
                header('Location: afficher.php');
            } 
            catch (Exception $e) 
            {
