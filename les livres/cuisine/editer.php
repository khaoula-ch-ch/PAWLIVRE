...
{
    header("Location: logincuisine.php");
}
...
if(isset($_GET['id'])){
    ...
    {
        $id = $_GET['id'];
        $lelivre = afficherUnProduitcuisine($id);
    }
}
...
<body>
    ...
    <a class="navbar-brand" href="cuisine.php">Les livres</a>
...
<?php foreach ($lelivre as $cuisine): ?>
        
    <form method="post">
    
    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Nom du produit</label>
        <input type="text" class="form-control" name="nom" value="<?= $cuisine->nom ?>"  required>
    </div>

    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Prix</label>
        <input type="number" class="form-control" name="prix" value="<?= $cuisine->prix ?>" required>
    </div>
    <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">L'image du produit</label>
        <input type="name" class="form-control" name="image" value="<?= $cuisine->image ?>" required>

    </div>
    <div class="mb-3">
        <label for="exampleInputPassword1" class="form-label">Description</label>
        <textarea class="form-control" name="description" required><?= $cuisine->description ?></textarea>
    </div>
...
<?php
    ...
    if(isset($_POST['valider']))
    {
        ...
        if( !empty($_POST['nom']) AND !empty($_POST['prix']) AND  !empty($_POST['image']) AND !empty($_POST['description']))
            ...
            try 
            {
                modifiercuisine( $nom, $prix, $image,  $description, $id);
                header('Location: afficher.php');
            } 
            catch (Exception $e) 
            {
