...
<html lang="en">
  ...
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-kenU1KFDBcuisineIe4zVF0s0G1M5b4hcpxyD9F7jL+jjXkk+Q2h455rYXK/7HAuoJl+0I4" crossorigin="anonymous"></script>
...
if(isset($_GET['id'])){
    $mesproduits = $DBcuisine->query('SELECT id FROM cuisine WHERE id=:id', array('id'=> $_GET['id']));
    if(empty($mesproduits)){
      die("ce produit n'exicte pas" );
    }
      $paniercuisine->add($mesproduits[0]->id); 
      die('le livre a bien ete ajouter aux paniercuisine <a href="javascript:history.back()" style="color : black">retourner sur laccueil</a>');
}else{
